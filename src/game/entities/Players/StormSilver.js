...
export default class StormSilver extends BasePlayer {
  ...
  update(time, delta) {
    ...
    if (this.body && Math.abs(this.body.velocity.x) > 120) {
  ...
  createHolographicLane(xOff, currentColor) {
    ...
    const lane = this.scene.bullets.create(
      this.x + xOff,
      this.y - 170,
      "base-rounded-bullet",
    );
    ...
    if (lane) {
      // Restore Projectile.js physics settings
      lane.body.allowGravity = false;
      lane.setDepth(5);
      ...
      const stripL = this.scene.add
        .sprite(-24, 0, "base-rounded-bullet")
        .setAlpha(0.3)
      ...
      const updateFX = () => {
        ...
        const scrollY = ((timeNow * 0.5) % 400) - 200;
        hA.x = Math.sin(timeNow * 0.008) * 20;
        hB.x = -hA.x;
        hA.y = hB.y = scrollY;
        ...
        const wave = Math.sin(timeNow / 150);
      ...
      // Custom Impact logic (if handled by your damage system)
      lane.onHit = (enemy) => {
  ...
  triggerImpactFX(x, y) {
    const emitter = this.scene.add.particles(x, y, "base-rounded-bullet", {
  ...
  executeMonolithBeam() {
    ...
    [0x00ffff, 0xff00ff, 0x00ffff].forEach((col, i) => {
      const s = this.scene.add
        .tileSprite((i - 1) * 70, 0, 20, 800, "base-rounded-bullet")
        .setTint(col)
        .setAlpha(0.7)
        .setBlendMode("ADD");
      strips.push(s);
  ...
  createGhostLaser(x, y) {
    const laser = this.scene.bullets.get(x, y, "base-rounded-bullet");
    if (laser) {
